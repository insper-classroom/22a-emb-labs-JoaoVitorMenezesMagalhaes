...
void lv_termostato(void) {
	...
	lv_obj_align(downBut, LV_ALIGN_BOTTOM_RIGHT, -5, -20);
	...
	lv_obj_align_to(upBtn, downBut ,LV_ALIGN_OUT_LEFT_MID, -25, -20/2);
	...
	lv_obj_align_to(labelFloorDecimal, labelFloor ,LV_ALIGN_OUT_RIGHT_MID, -0 , 10);
	...
	lv_label_set_text_fmt(labelFloorDecimal, ".%d", 8);
	...
	lv_label_set_text_fmt(labelTime, "%02d:42", 15);
	
	labelSetValue = lv_label_create(lv_scr_act());
	lv_obj_align_to(labelSetValue, labelTime ,LV_ALIGN_OUT_BOTTOM_MID, -10 , 10);
	...
	lv_label_set_text_fmt(labelSetValue, "%02d", 24);
...
static void task_lcd(void *pvParameters) {
	...
	for (;;)  {
		if (xSemaphoreTake(xSemaphorePower, 1000)) {
			is_sleeping = 1;
		}
...
static void task_power(void *pvParameters){
